/*
    [Rotinas internas]    
    - Pode ser que uma determinada rotina eu quero que sejá acessivel somente
    dentro do pacote e não do lado de fora dele.
    - Para isto podemos criar algumas rotinas internas, ou seja uma função ou
    procedure que seja acessivel somente dentro do pacote e não pelo lado de fora.
    - Para isto basta adicionar a procedure/função no corpo do pacote e não 
    adicionar no cabecalho.
    
    - Quando adicionamos a procedure no cabeçalho somos obrigados a adicionar no
      corpo, mais o contrario não precisa acontecer.
    - O cabecalho da procedure serve somente para tornar publico os metodos
      internos do corpo.
    - O cabecalho é semelhante ao "export do javascript", que utilizamos para
